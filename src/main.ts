...
const HELLO_TEXTS: { [language: string]: string } = {
    en: 'Hello',
    de: 'Hallo',
    pl: 'Cześć',
    uk: 'Привіт',
 ...
 */
const TEST_TEXTS: { [language: string]: string } = {
...
const GENERATED_TEXTS: Record<ioBroker.Languages, string> = {
...
export class SayItAdapter extends Adapter {
     ...
     */
    getEngineLanguage(engine: EngineType, freettsVoice?: string): string {
        // The FreeTTS engine has no own language. It is defined by the voice, like "de-DE-KatjaNeural"
        return (engine === 'freeTTS' ? freettsVoice || this.config.freettsVoice || 'en' : (engine as string)).substring(
            0,
            2,
        );
    }
     ...
     */
    getHelloText(engine: EngineType, freettsVoice?: string): string {
     ...
     */
    getTestText(engine: EngineType, freettsVoice?: string): string {
     ...
     */
    async testGenerate(obj: ioBroker.Message): Promise<void> {
        if (!this.text2speech) {
            this.sendTo(obj.from, obj.command, { error: 'Adapter is not ready yet' }, obj.callback);
            return;
        }

        const testOptions: TestOptions = { ...obj.message } as TestOptions;
        const engine: EngineType = testOptions.engine || this.config.engine || 'en';
        const text =
            ((obj.message?._testText as string) || '').trim() || this.getTestText(engine, testOptions.freettsVoice);
        ...
        const maxLength = Text2Speech.getMaxTextLength(engine);
        const parts = maxLength ? Text2Speech.splitText(text, maxLength) : [text];
        ...
        try {
            for (const part of parts) {
                ...
                audio.push(readFileSync(fileName));
            }
        } catch (e) {
        ...
        const { duration, url } = await this.storeTestFile(generated, engine, obj.message?._origin as string);
    ...
    processMessage(obj: ioBroker.Message): void {
        if (obj.command === 'say') {
            ...
            const engine = (obj.message?.engine as EngineType) || this.config.engine || 'en';
            const text = `${this.getHelloText(engine, obj.message?.freettsVoice as string)} ${this.helloCounter++}`;
