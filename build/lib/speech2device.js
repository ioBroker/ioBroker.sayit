...
class Speech2Device {
    ...
    async #uploadToStates(text, saveToDisk) {
        ...
        if (data) {
            if (saveToDisk) {
                try {
                    (0, node_fs_1.writeFileSync)(this.MP3FILE, data);
                }
                catch (e) {
        ...
        else if (_a.isPlayFile(text)) {
            fileName = (0, node_path_1.normalize)(text);
        }
        else {
            fileName = this.MP3FILE;
        }
        try {
            fileData = (0, node_fs_1.readFileSync)(fileName);
        }
        catch (e) {
    ...
    async #sayItBrowser(props) {
        ...
        const browserVis = props.testOptions?.browserVis === undefined ? this.config.browserVis : props.testOptions.browserVis;
        ...
        if (!browserVis || browserVis === '2') {
            try {
                ...
                if (this.vis2exist) {
                    await this.adapter.setForeignStateAsync('vis-2.0.control.command', JSON.stringify({
                        command: 'playSound',
    ...
    #launchGoogleHome(client, url) {
        return new Promise((resolve, reject) => client.launch(castv2_client_1.DefaultMediaReceiver, (err, player) => {
            if (!player) {
    ...
    async #sendToGoogleHome(host, url, volume) {
        ...
        return new Promise((resolve, reject) => {
            client.connect(host, async () => {
                if (volume !== undefined) {
                    try {
                        await new Promise((_resolve, _reject) => client.setVolume({ level: volume / 100 }, (err) => err ? _reject(new Error(err.toString())) : _resolve()));
                    }
                    catch (err) {
    ...
    async #sayItMP24(props) {
        const mp24Server = props.testOptions?.mp24Server || this.config.mp24Server;
        if (mp24Server) {
            if (!_a.isPlayFile(props.text)) {
                // say
                this.adapter.log.debug(`Request MediaPlayer24 "http://${mp24Server}:50000/tts=${encodeURI(props.text)}"`);
                try {
                    const response = await axios_1.default.get(`http://${mp24Server}:50000/tts=${encodeURI(props.text)}`);
                ...
                // play local file
                try {
                    const response = await axios_1.default.get(`http://${mp24Server}:50000/track=${props.text}`);
                    this.adapter.log.debug(`Response from MediaPlayer24 "${mp24Server}": ${response.data}`);
                }
                catch (e) {
                    if (e.message === 'Parse Error') {
                ...
                try {
                    ...
                    ftp.put(result.fileOnDisk, fileNameOnFTP, hadError => {
                        ...
                        ftp.raw('quit', async (err) => {
                            ...
                            if (!hadError) {
                                try {
                                    const response = await axios_1.default.get(`http://${mp24Server}:50000/track=${fileNameOnFTP}`);
                                    this.adapter.log.debug(`Response from MediaPlayer24 "${mp24Server}": ${response.data}`);
                                    resolve();
                                }
                                catch (e) {
    ...
    async #sayItSystem(props) {
        ...
        if (result.fileOnDisk) {
            if (systemCommand) {
                ...
                // windows
                try {
                    await this.#exec('cmdmp3.exe', [`"${result.fileOnDisk}"`], (0, node_path_1.normalize)(`${__dirname}/../cmdmp3/`));
                }
                catch (e) {
                    this.adapter.log.error(`Cannot play:${e}`);
                    return false;
    ...
    async #sayItWindows(props) {
        ...
        if (p.match(/^win/)) {
            // windows
            try {
                await this.#exec((0, node_path_1.normalize)(`${__dirname}/../say/SayStatic.exe`), [`"${props.text}"`]);
            }
            catch (error) {
    ...
    async sayItSystemVolume(level, testOptions) {
        ...
        if (p === 'linux' && systemPlayer !== 'mpg321') {
            ...
            try {
                await this.#spawn((0, node_path_1.normalize)(`${__dirname}/../nircmd/nircmdc.exe`), ['setsysvolume', level.toString()], true);
            }
            catch {
