...
function Speech2Device(adapter, libs, options) {
    ...
    var MP3FILE = path.normalize(__dirname + '/../' + adapter.namespace + '.say.'+ options.outFileExt);
    const that = this;
    ...
    function sayItSonos(error, text, language, volume, duration, callback) {
        ...
        uploadToStates(text, err => {
            ...
            if (adapter.config.device && options.webLink) {
                adapter.log.info('Set "' + adapter.config.device + '.tts: ' + (volume ? (volume + ';') : '') + options.webLink + '/state/' + adapter.namespace + '.tts.'+options.outFileExt);
                adapter.setForeignState(adapter.config.device + '.tts', (volume ? (volume + ';') : '') + options.webLink + '/state/' + adapter.namespace + '.tts.'+options.outFileExt);
            } else if (options.webLink) {
                adapter.log.info('Send to sonos ' + (volume ? (volume + ';') : '') + options.webLink + '/state/' + adapter.namespace + '.tts.'+options.outFileExt);
                adapter.sendTo('sonos', 'send', (volume ? (volume + ';') : '') + options.webLink + '/state/' + adapter.namespace + '.tts.'+options.outFileExt);
            } else {
    ...
    function sayItMpd(error, text, language, volume, duration, callback) {
        ...
        uploadToStates(text, err => {
            ...
            if (adapter.config.mpd_device && options.webLink) {
                adapter.log.info('Set "' + adapter.config.mpd_device + '.say: ' + (volume ? (volume + ';') : '') + options.webLink + '/state/' + adapter.namespace + '.tts.'+options.outFileExt);
                adapter.setForeignState(adapter.config.mpd_device + '.say', (volume ? (volume + ';') : '') + options.webLink + '/state/' + adapter.namespace + '.tts.'+options.outFileExt);
            } else if (options.webLink) {
                adapter.log.info('Send to MPD ' + (volume ? (volume + ';') : '') + options.webLink + '/state/' + adapter.namespace + '.tts.'+options.outFileExt);
                adapter.sendTo('mpd', 'say', (volume ? (volume + ';') : '') + options.webLink + '/state/' + adapter.namespace + '.tts.'+options.outFileExt);
            } else {
                adapter.log.warn('Web server is unavailable!');
            }
            callback && callback(null, duration);
        ...
        uploadToStates(text, err => {
            ...
            if (adapter.config.port && adapter.config.server) {
                ...
                try {
                    ...
                    ftp.put(file, adapter.namespace + '.say.'+options.outFileExt, hadError => {
                        if (!hadError) {
                            const opts = {
                                host: adapter.config.server,
                                port: 50000,
                                path: '/track=' + adapter.namespace + '.say.'+options.outFileExt
                            };
    ...
    function sayItSystem(error, text, language, volume, duration, callback) {
        ...
        uploadToStates(text, err => {
            ...
            if (ls) {
                ls.on('error', e => {
                    throw new Error('sayIt.play: there was an error while playing the file:' + e.toString);
                });
            }
