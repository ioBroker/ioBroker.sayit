...
function Speech2Device(adapter, libs, options) {
    const sayitFuncs = {
        ...
        heos:       { func: sayItHeos },
        lametric:   { func: sayItLaMetric },
        chromecast: { func: sayItChromecast },
        mpd:        { func: sayItMpd },
        googleHome: { func: sayItGoogleHome }
    };

    const MP3FILE = path.normalize(`${adapter.dataDir}/${adapter.namespace}.say.${options.outFileExt}`);
    const that = this;

    function uploadToFiles(text, callback) {
        let fileData;
        ...
        if (isPlayFile(text)) {
            ...
            try {
                ...
                return callback && callback(`Cannot upload file "${text}" to state: ${e.toString()}`);
            }
        } else {
            try {
                fileData = libs.fs.readFileSync(MP3FILE);
            } catch (e) {
                return callback && callback(`Cannot upload file "${MP3FILE}" to state: ${e.toString()}`);
            }
        }

        adapter.writeFile(adapter.namespace, `tts.${options.outFileExt}`, fileData, callback);
    }
    ...
    function sayItBrowser(error, text, language, volume, duration, callback) {
        ...
        uploadToFiles(text, err => {
            if (err) {
                return callback && callback(err);
            } else {
                adapter.setForeignState('vis.0.control.instance', adapter.config.browserInstance);
    ...
    function sayItSonos(error, text, language, volume, duration, callback) {
        ...
        uploadToFiles(text, err => {
            ...
            const fileName = `${volume ? (volume + ';') : ''}${options.webLink}/${adapter.namespace}/tts.${options.outFileExt}`;

            if (adapter.config.deviceSonos && options.webLink) {
                adapter.log.info(`[sayItSonos] Setting "${adapter.config.deviceSonos}.tts" to "${fileName}"`);
                adapter.setForeignState(`${adapter.config.deviceSonos}.tts`, fileName);
            } else if (options.webLink) {
                adapter.log.info(`[sayItSonos] Sending file "${fileName}"`);
                adapter.sendTo('sonos', 'send', fileName);
            } else {
                adapter.log.warn('[sayItSonos] Web server is unavailable!');
            }
            callback && callback(null, duration);
        });
    }
    ...
    function sayItHeos(error, text, language, volume, duration, callback) {
        ...
        uploadToFiles(text, err => {
            ...
            if (adapter.config.deviceHeos && options.webLink) {
                adapter.log.info(`[sayItHeos] Setting "${adapter.config.deviceHeos}.tts" to "${fileName}"`);
                adapter.setForeignState(adapter.config.deviceHeos + '.tts', fileName);
            } else if (options.webLink) {
                adapter.log.info(`[sayItHeos] Sending file "${fileName}"`);
                adapter.sendTo('heos', 'send', fileName);
            } else {
                adapter.log.warn('[sayItHeos] Web server is unavailable!');
            }
            callback && callback(null, duration);
        });
    }

    function sayItLaMetric(error, text, language, volume, duration, callback) {
        if (error) {
            return callback && callback(error, 0);
        }
        ...
        uploadToFiles(text, err => {
            ...
            const fileName = `${options.webLink}/${adapter.namespace}/tts.${options.outFileExt}`;

            if (adapter.config.deviceLametric && options.webLink) {
                adapter.log.info(`[sayItLaMetric] Sending file "${fileName}"`);

                adapter.sendTo(
                    adapter.config.deviceLametric,
                    'notification',
                    {
                        priority: 'info',
                        iconType: 'none',
                ...
                adapter.log.warn('[sayItLaMetric] Web server is unavailable!');
            }
            callback && callback(null, duration);
        });
    }
    ...
    function sayItMpd(error, text, language, volume, duration, callback) {
        ...
        uploadToFiles(text, err => {
            ...
            if (adapter.config.deviceMpd && options.webLink) {
                adapter.log.info('Set "' + adapter.config.deviceMpd + '.say: ' + fileName);
                adapter.setForeignState(adapter.config.deviceMpd + '.say', fileName);
            } else if (options.webLink) {
    ...
    function sayItChromecast(error, text, language, volume, duration, callback) {
        ...
        uploadToFiles(text, err => {
            ...
            const announcement = {
                url: `${options.webLink}/${adapter.namespace}/tts.${options.outFileExt}`
            };
    ...
    function launchGoogleHome(client, url, callback) {
        client.launch(DefaultMediaReceiver, (err, player) => {
            if (!player) {
                return adapter.log.warn('Player not available.');
            }
            const media = {
                contentId: url,
        ...
        client.connect(host, () => {
            if (volume !== undefined) {
                client.setVolume({ level: volume / 100 }, (err /* , newvol */) => {
                    err && adapter.log.error('there was an error setting the volume: ' + err);
    ...
    function sayItGoogleHome(error, text, language, volume, duration, callback) {
        ...
        uploadToFiles(text, err => {
            ...
            if (adapter.config.serverGoogleHome && options.webLink) {
                const url = `${options.webLink}/${adapter.namespace}/tts.${options.outFileExt}`;
                adapter.log.debug(`Send to google home "${adapter.config.serverGoogleHome}": ${url}`);

                sendToGoogleHome(adapter.config.serverGoogleHome, url, volume, err => err && adapter.log.error(err));
            } else {
    ...
    function sayItMP24(error, text, language, volume, duration, callback) {
        ...
        if (adapter.config.serverMp24) {
            if (!isPlayFile(text)) {
                // say
                adapter.log.debug(`Request MediaPlayer24 "http://${adapter.config.serverMp24}:50000/tts=${encodeURI(text)}"`);
                const opts = {
                    host: adapter.config.serverMp24,
                    port: 50000,
                ...
                libs.http.get(opts, res => {
                    ...
                    res.on('end', () => adapter.log.debug(`Response from MediaPlayer24 "${adapter.config.serverMp24}": ${body}`));
                    res.on('error', e => adapter.log.error(`Cannot say text on MediaPlayer24 "${adapter.config.serverMp24}": ${e.message}`));
                }).on('error', e => {
                    if (e.message === 'Parse Error') {
                        adapter.log.debug('Played successfully');
                    } else {
                        error = `Cannot say text on MediaPlayer24 "${adapter.config.serverMp24}":${e.message}`;
                    }
                });
            } else {
                // play local file
                ...
                libs.http.get(opts, res => {
                    ...
                    // all data has been downloaded
                    res.on('end', () => adapter.log.debug(`Response from MediaPlayer24 "${adapter.config.serverMp24}": ${body}`));
                    res.on('error', e => adapter.log.error(`Cannot play local file on MediaPlayer24 "${adapter.config.serverMp24}":${e.message}`));
                }).on('error', e => {
                    if (e.message === 'Parse Error') {
                        adapter.log.debug('Played successfully');
                    } else {
                        adapter.log.error(`Cannot play local file on MediaPlayer24 "${adapter.config.serverMp24}":${e.message}`);
                    }
                });
            }
        }
    ...
    function sayItMP24ftp(error, text, language, volume, duration, callback) {
        ...
        uploadToFiles(text, err => {
            ...
            if (adapter.config.mp24Port && adapter.config.serverMp24) {
                ...
                const ftp = new libs.jsftp({
                    host: adapter.config.serverMp24,
                    port: parseInt(adapter.config.mp24Port, 10), // defaults to 21
                    user: adapter.config.mp24User || 'anonymous', // defaults to 'anonymous'
                    pass: adapter.config.mp24Pass || 'anonymous'  // defaults to 'anonymous'
                });
                ...
                try {
                        ...
                        if (!hadError) {
                            ...
                            libs.http.get(opts, res => {
                                ...
                                if (e.message === 'Parse Error') {
                                    ...
                                    adapter.log.error(`Cannot say text on MediaPlayer24 "${adapter.config.serverMp24}":${e.message}`);
                                }
                            });
                        } else {
                    ...
                    error = `Cannot upload file to ${adapter.config.serverMp24}:${adapter.config.mp24Port}`;
                }
            }
            callback && callback(error, duration + 2);
        });
    ...
    function sayItSystem(error, text, language, volume, duration, callback) {
        ...
        uploadToFiles(text, err => {
            ...
            if (adapter.config.command) {
                //custom command
                adapter.setState('tts.playing', { val: true, ack: true });
                if (adapter.config.command.includes('%s')) {
                ...
                ls = libs.child_process.exec(cmd, (error /* , stdout, stderr */) => {
                    error && adapter.log.error('Cannot play:' + error);
                    adapter.setState('tts.playing', { val: true, ack: true });
                });
            } else {
                if (p === 'linux') {
    ...
    function sayItWindows(error, text, language, volume, duration, callback) {
        ...
        // If mp3 file
        if (isPlayFile(text)) {
            return sayItSystem(text, language, volume);
        }
        ...
        if (p.match(/^win/)) {
            ...
            ls = libs.child_process.exec(`${__dirname}/../say/SayStatic.exe ${text}`, (error /* , stdout, stderr */) => {
                error && adapter.log.error('sayItWindows: ' + error);
                adapter.setState('tts.playing', { val: false, ack: true });
            });
        } else {
    ...
    this.sayItSystemVolume = function (level) {
        ...
        if (level === options.sayLastVolume) {
            return;
        }
        libs.os = libs.os || require('os');
