...
class Speech2Device {
    ...
    async uploadToStates(text) {
        ...
        return {fileInDB: `/${this.adapter.namespace}/${file}?ts=${Date.now()}`, fileOnDisk: fileName};
    }
    ...
    async sayItBrowser(text, language, volume, duration, testOptions) {
        ...
        const browserVis = (testOptions && testOptions.browserVis) || this.adapter.config.browserVis;

        // check if vis.0.control.command exists
        if (!browserVis || browserVis === 1 || browserVis === '1') {
            try {
                if (this.vis1exist === null) {
                    this.vis1exist = !!(await this.adapter.getObjectAsync('vis.0.control.command'));
                    if (!this.vis1exist) {
        ...
        if (!browserVis || browserVis === 2 || browserVis === '2') {
            try {
                if (this.vis2exist === null) {
                    this.vis2exist = !!(await this.adapter.getObjectAsync('vis-2.0.control.command'));
                    if (!this.vis2exist) {
