...
function Speech2Device(adapter, libs, options) {
    ...
    function uploadToStates(text, callback) {
        ...
        if (adapter.setForeignBinaryState) {
            adapter.setForeignBinaryState(`${adapter.namespace}.tts.${options.outFileExt}`, fileData, callback);
            adapter.log.debug(`[uploadToStates] Saved generated ${options.outFileExt} file to ${adapter.namespace}.tts.${options.outFileExt}`);
        } else {
    ...
    function sayItLaMetric(error, text, language, volume, duration, callback) {
        ...
        uploadToStates(text, err => {
            ...
            const fileName = `${options.webLink}/state/${adapter.namespace}.tts.${options.outFileExt}`;
