...
class Speech2Device {
    constructor(adapter, options) {
        ...
        this.MP3FILE = path.normalize(`${this.adapter.config.dataDir}/${this.adapter.namespace}.say.${options.outFileExt}`);
        // Google home
        this.GHClient = null;
    ...
    async sayItBrowser(text, language, volume, duration) {
        ...
        await this.adapter.setForeignStateAsync('vis.0.control.instance', this.adapter.config.instance);
    ...
    async sayItSonos(text, language, volume, duration) {
        ...
        if (this.adapter.config.device && this.options.webLink) {
            this.adapter.log.info(`Set "${this.adapter.config.device}.tts: ${fileName}`);
            await this.adapter.setForeignStateAsync(`${this.adapter.config.device}.tts`, fileName);
    ...
    async sayItHeos(text, language, volume, duration) {
        ...
        if (this.adapter.config.heos_device && this.options.webLink) {
            this.adapter.log.info(`Set "${this.adapter.config.heos_device}.tts: ${fileName}`);
            await this.adapter.setForeignStateAsync(`${this.adapter.config.heos_device}.tts`, fileName);
        } else if (this.options.webLink) {
    ...
    async sayItMpd(text, language, volume, duration) {
        ...
        if (this.adapter.config.mpd_device && this.options.webLink) {
            this.adapter.log.info(`Set "${this.adapter.config.mpd_device}.say: ${fileName}`);
            await this.adapter.setForeignStateAsync(`${this.adapter.config.mpd_device}.say`, fileName);
        } else if (this.options.webLink) {
    ...
    async sayItChromecast(text, language, volume, duration) {
        ...
        if (this.adapter.config.cDevice && this.options.webLink) {
            const chromecastAnnouncementDev = `${this.adapter.config.cDevice}.player.announcement`;
    ...
    async sayItGoogleHome(text, language, volume, duration) {
        ...
        // version 1.x stored server in server
        // version 2.x stores server in ghServer
        const server = this.adapter.config.ghServer || this.adapter.config.server;

        if (server && this.options.webLink) {
            const url = `${this.options.webLink}${result.fileInDB}`;
            this.adapter.log.debug(`Send to google home "${server}": ${url}`);

            await this.sendToGoogleHome(server, url, volume);
        } else {
    ...
    async sayItMP24(text, language, volume, duration) {
        ...
        if (this.adapter.config.server) {
			if (!Speech2Device.isPlayFile(text)) {
				// say
				this.adapter.log.debug(`Request MediaPlayer24 "http://${this.adapter.config.server}:50000/tts=${encodeURI(text)}"`);
                try {
                    const response = await axios.get(`http://${this.adapter.config.server}:50000/tts=${encodeURI(text)}`);
                    this.adapter.log.debug(`Response from MediaPlayer24 "${this.adapter.config.server}": ${response.data}`);
                } catch (e) {
                    if (e.message === 'Parse Error') {
                        this.adapter.log.debug('Played successfully');
                    } else {
                        this.adapter.log.error(`Cannot say text on MediaPlayer24 "${this.adapter.config.server}": ${e.message}`);
                        throw e;
                    }
                }
			} else {
				// play local file
                try {
                    const response = await axios.get(`http://${this.adapter.config.server}:50000/track=${text}`);
                    this.adapter.log.debug(`Response from MediaPlayer24 "${this.adapter.config.server}": ${response.data}`);
                } catch (e) {
                    if (e.message === 'Parse Error') {
                        this.adapter.log.debug('Played successfully');
    ...
    async sayItMP24ftp(text, language, volume, duration) {
        ...
        if (this.adapter.config.port && this.adapter.config.server) {
            jsftp = jsftp || require('jsftp');
            axios = axios || require('axios');

            const ftp = new jsftp({
                host: this.adapter.config.server,
                port: parseInt(this.adapter.config.port, 10), // defaults to 21
                user: this.adapter.config.user || 'anonymous', // defaults to 'anonymous'
                pass: this.adapter.config.pass || 'anonymous',  // defaults to 'anonymous'
            });

            await new Promise((resolve, reject) => {
                try {
                    // Copy file to FTP server
                    const fileName = `${this.adapter.namespace}.say.${this.options.outFileExt}`;
                    ftp.put(result.fileOnDisk, fileName, hadError => {
                        // send quit command
                        ftp.raw('quit', async err => { // , data
                            ...
                            if (!hadError) {
                                try {
                                    const response = await axios.get(`http://${this.adapter.config.server}:50000/track=${fileName}`);
                                    this.adapter.log.debug(`Response from MediaPlayer24 "${this.adapter.config.server}": ${response.data}`);
                                    resolve();
                                } catch (e) {
                                    if (e.message === 'Parse Error') {
                    ...
                    throw new Error(`Cannot upload file to ${this.adapter.config.server}:${this.adapter.config.port}`);
                }
            });
        }
    ...
    async sayItSystem(text, language, volume, duration) {
        ...
        if (this.adapter.config.command) {
            // custom command
            if (this.adapter.config.command.includes('%s')) {
                cmd = this.adapter.config.command.replace('%s', result.fileOnDisk);
            } else {
                if (p.match(/^win/)) {
                    cmd = `${this.adapter.config.command} "${result.fileOnDisk}"`;
                } else {
                    cmd = `${this.adapter.config.command} ${result.fileOnDisk}`;
                }
            }
            ...
            if (p === 'linux') {
                // linux
                if (this.adapter.config.player === 'omxplayer') {
                    cmd = `omxplayer -o local ${result.fileOnDisk}`;
                } else if (this.adapter.config.player === 'mpg321') {
                ...
                // mac osx
                try {
                    await this.exec('/usr/bin/afplay', [result.fileOnDisk], path.normalize(`${__dirname}/../cmdmp3/`));
                } catch (e) {
                    this.adapter.log.error(`Cannot play:${e}`);
                }
            }
    ...
    async sayItWindows(text, language, volume, duration) {
        ...
        if (p.match(/^win/)) {
            // windows
            this.adapter.setState('tts.playing', true, true);
