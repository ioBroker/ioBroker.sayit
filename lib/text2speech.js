...
class Text2Speech {
    ...
    sayItGetSpeechPolly(text, language, _, testOptions) {
        aws = aws || require('aws-sdk');
        const awsAccessKey = (testOptions && testOptions.awsAccessKey) || this.adapter.config.awsAccessKey;
        const awsSecretKey = (testOptions && testOptions.awsSecretKey) || this.adapter.config.awsSecretKey;
        const awsRegion = (testOptions && testOptions.awsRegion) || this.adapter.config.awsRegion;
    ...
    async sayItGetSpeechCloud(text, language, _, testOptions) {
        ...
        if (!this.cloudAppKey) {
            ...
            const obj = await this.adapter.getForeignObjectAsync(`system.this.adapter.${cloudInstance}`);
