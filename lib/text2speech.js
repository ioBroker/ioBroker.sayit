...
class Text2Speech {
    ...
    async sayItGetSpeechYandexCloud(text, language) {
        ...
        const voice = this.adapter.config.yandexCloudVoice || '';
        const params = new URLSearchParams();
        params.append('text', text);
        params.append('voice', voice.replace(' Premium', ''));
        params.append('folderId', this.adapter.config.yandexFolderID || '');
        if (this.adapter.config.yandexEmotion !== 'none' && this.adapter.config.yandexEmotion) {
            params.append('emotion', this.adapter.config.yandexEmotion);
        }
        params.append('lang', language);

        const response = await axios.post(`https://tts.api.cloud.yandex.net/speech/v1/tts:synthesize`, params, {
            headers: {
                'Authorization': `Api-Key ${this.adapter.config.yandexKey}`,
            },
    ...
    async sayItGetSpeechYandex(text, language) {
        ...
        */
        ...
        let url = `https://tts.voicetech.yandex.net/generate?lang=${language}&format=mp3&speaker=${this.adapter.config.yandexVoice}&key=${this.adapter.config.yandexKey}&text=${encodeURI(text.trim())}`;

        if (this.adapter.config.yandexEmotion && this.adapter.config.yandexEmotion !== 'none') {
            url += `&emotion=${this.adapter.config.yandexEmotion}`;
        }
        if (this.adapter.config.yandexDrunk === 'true' || this.adapter.config.yandexDrunk === true) {
            url += '&drunk=true';
        }
        if (this.adapter.config.yandexIll   === 'true' || this.adapter.config.yandexIll   === true) {
            url += '&ill=true';
        }
        if (this.adapter.config.yandexRobot === 'true' || this.adapter.config.yandexRobot === true) {
            url += '&robot=true';
        }
    ...
    async sayItGetSpeechCoquiTTS(text, language) {
        language = language.substring(0, 2);
        let cmd;
        const coquiVocoder = this.adapter.config.coquiVocoder;
    ...
    async sayItGetSpeechCloud(text, language) {
        axios = axios || require('axios');

        if (!this.cloudAppKey) {
            const obj = await this.adapter.getForeignObjectAsync(`system.this.adapter.${this.adapter.config.cloudInstance}`);
            this.cloudAppKey = obj && obj.native && obj.native.apikey ? obj.native.apikey : 'error';
            this.cloudUrl = (obj && obj.native && obj.native.cloudUrl) || '';
            if (this.cloudAppKey && this.cloudAppKey.match(/^@pro_/)) {
        ...
        if (this.cloudAppKey === 'error') {
            throw new Error(`No app key found in "${this.adapter.config.cloudInstance}".`);
        }
        ...
        const params = new URLSearchParams();
        params.append('Text', text);
        params.append('appkey', this.cloudAppKey);
    ...
    async sayItGetSpeech(text, language, volume) {
        ...
        if (sayitEngines[language] && sayitEngines[language].engine) {
            ...
            if (engine === 'google') {
                ...
                await this.sayItGetSpeechCoquiTTS(text, language, volume);
