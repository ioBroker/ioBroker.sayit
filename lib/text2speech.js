...
function Text2Speech(adapter, libs, options, sayIt) {
    ...
    const MP3FILE      = `${adapter.dataDir}/${adapter.namespace}.say.${options.outFileExt}`;
    let appkey         = null;
    let cloudUrl       = null;
    let polly          = null;
    ...
    function sayItGetSpeechYandexCloud(text, language, volume, callback) {
        ...
        if (!yandexCloudIAM) {
            yandexCloudIAM = yandexGetIAM(adapter.config.yandexKey);
            adapter.log.debug('Get IAM token for ' + adapter.config.yandexKey);
        }

        yandexCloudIAM(IAM => {
            let voice = adapter.config.effectVoice || '';

            const postData = {
                voice: voice.replace(' Premium', ''),
                emotion: adapter.config.effectEmotion,
                folderId: adapter.config.yandexFolderID,
                text
            };
    ...
    function sayItGetSpeechYandex(text, language, volume, callback) {
        ...
        */
        const options = {
            ...
            path: `/generate?lang=${language}&format=mp3&speaker=${adapter.config.effectVoice}&key=${adapter.config.yandexKey}&text=${encodeURI(text.trim())}`
        };

        if (adapter.config.effectEmotion && adapter.config.effectEmotion !== 'none') options.path += '&emotion=' + adapter.config.effectEmotion;
        if (adapter.config.effectDrunk === 'true' || adapter.config.effectDrunk === true) options.path += '&drunk=true';
        if (adapter.config.effectIll   === 'true' || adapter.config.effectIll   === true) options.path += '&ill=true';
        if (adapter.config.effectRobot === 'true' || adapter.config.effectRobot === true) options.path += '&robot=true';
    ...
    function sayItGetSpeechPolly(text, language, volume, callback) {
        ...
        try {
            polly = polly || new libs.aws.Polly({
                accessKeyId:     adapter.config.awsAccessKey,
                secretAccessKey: adapter.config.awsSecretKey,
                region:          adapter.config.awsRegion
            });

            adapter.log.debug(`[sayItGetSpeechPolly] Access Key: "${String(adapter.config.awsAccessKey).substring(0, 5)}*****", Secret: "${String(adapter.config.awsSecretKey).substring(0, 5)}*****", Region: "${adapter.config.awsRegion}"`);
    ...
    function sayItGetSpeechPollyNeural(text, language, volume, callback) {
        libs.aws = libs.aws || require('aws-sdk');
        libs.fs  = libs.fs  || require('fs');
        ...
        try {
            ...
            adapter.log.debug(`[sayItGetSpeechPollyNeural] Access Key: "${String(adapter.config.awsAccessKey).substring(0, 5)}*****", Secret: "${String(adapter.config.awsSecretKey).substring(0, 5)}*****", Region: "${adapter.config.awsRegion}"`);
    ...
    function sayItGetSpeechCloud(text, language, volume, callback) {
        ...
        if (!appkey) {
            adapter.getForeignObject('system.adapter.' + adapter.config.cloudInstance, (err, obj) => {
        ...
        if (appkey === 'error') {
            callback && callback(`No app key found in "${adapter.config.cloudInstance}".`, text, language, volume, 0);
            return;
        }
    ...
    const ENGINES = {
        ...
        'pollyNeural': sayItGetSpeechPollyNeural,
    ...
    this.sayItGetSpeech = function (text, language, volume, callback) {
        if (adapter.config.cache) {
            ...
            adapter.log.debug(`[sayItGetSpeech] Cache file ${md5filename} for "${language};${text}" not found`);
        }
