...
function Text2Speech(adapter, libs, options, sayIt) {
    ...
    function sayItGetSpeechCoquiTTS(text, language, volume, callback) {
        ...
        try {
            let cmd;
                if(adapter.config.vocoder === 'default'){
                    cmd = `tts --text ". ${text} ." --model_name  tts_models/${language}/${adapter.config.vocoder.replace(' ', '/')} --out_path ${__dirname}/say.wav`;
                    console.log(cmd);
                }else{
                    if(adapter.config.vocoder === 'libri-tts wavegrad' || adapter.config.vocoder === 'libri-tts fullband-melgan'){
                        language = 'universal';
                    }
                    cmd = `tts --text ". ${text} ." --model_name  tts_models/${language}/${adapter.config.vocoder.replace(' ', '/')} --vocoder_name vocoder_models/${language}/${adapter.config.vocoder.replace(' ', '/')} --out_path ${__dirname}/say.wav`;
                }
            libs.exec(cmd, (error, stdout, stderr) => {
                if (error) {
