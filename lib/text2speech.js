...
class Text2Speech {
    ...
    async sayItGetSpeechGoogle(text, language, volume, testOptions) {
        ...
        if (text.length > 70) {
            ...
            try {
                for (let t = 1; t < parts.length; t++) {
                    await this.addToQueue(parts[t], language, volume, testOptions);
                }
            } catch (error) {
        ...
        language ||= (testOptions && testOptions.engine) || this.adapter.config.engine;
    ...
    async sayItGetSpeechYandexCloud(text, language, _, testOptions) {
        ...
        axios ||= require('axios');
        const yandexCloudVoice =
            (testOptions && testOptions.yandexCloudVoice) || this.adapter.config.yandexCloudVoice || '';
    ...
    async sayItGetSpeechYandex(text, language, _, testOptions) {
        ...
        */
        ...
        const yandexEmotion = (testOptions && testOptions.yandexEmotion) || this.adapter.config.yandexEmotion;
        const yandexDrunk =
            (testOptions && testOptions.yandexDrunk !== undefined
                ? testOptions.yandexDrunk
                : this.adapter.config.yandexDrunk) || this.adapter.config.yandexDrunk;
        const yandexIll =
            (testOptions && testOptions.yandexIll !== undefined
                ? testOptions.yandexIll
                : this.adapter.config.yandexIll) || this.adapter.config.yandexIll;
        const yandexRobot =
            (testOptions && testOptions.yandexRobot !== undefined
                ? testOptions.yandexRobot
                : this.adapter.config.yandexRobot) || this.adapter.config.yandexRobot;
    ...
    async sayItGetSpeech(text, language, volume, testOptions) {
        if (this.adapter.config.cache && !testOptions) {
            const md5filename = this.isCached(
                this.options.cacheDir,
                `${language};${text}`,
                this.options.outFileExt,
                this.adapter.config.cacheExpiryDays,
            );
        ...
        if (this.adapter.config.cache) {
            this.cacheFile(
                text,
                language,
                this.getCachedFileName(this.options.cacheDir, `${language};${text}`, this.options.outFileExt),
            );
        }
